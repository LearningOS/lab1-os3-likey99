...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        task0.task_start_time = get_time_us(); //    lab1
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.tasks[next].task_start_time == 0{               //lab1
    ...
    fn get_current_taskinfo(&self)->TaskInfo{
        ...
        TaskInfo{
            ...
            time: (get_time_us()-inner.tasks[current].task_start_time)/1000,
        }

    }
    fn update_task_syscall(&self, syscall_id: usize){
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
...
pub fn taskinfo_from_tm()->TaskInfo{
    TASK_MANAGER.get_current_taskinfo()

}
pub fn update_task_syscall(syscall_id:usize){
    TASK_MANAGER.update_task_syscall(syscall_id);
}